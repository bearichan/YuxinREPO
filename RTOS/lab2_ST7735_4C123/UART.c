...
Sema4Type RxFIFO_semaFull, TxFIFO_semaFull;   //dataRoomLeft
Sema4Type RxFIFO_semaEmpty, TxFIFO_semaEmpty;   //dataAvailable
Sema4Type RxFIFO_semaMutex, TxFIFO_semaMutex;
...
void UART_Init(void){
  ...
  GPIO_PORTA_AMSEL_R = 0;               // disable analog functionality on PA
                                        // UART0=priority 2
  NVIC_PRI1_R = (NVIC_PRI1_R&0xFFFF00FF)|0x00004000; // bits 13-15  int NO.5 priority as 2
  NVIC_EN0_R = NVIC_EN0_INT5;           // enable interrupt 5 in NVIC
  
  //semaphore init
  OS_InitSemaphore(&RxFIFO_semaFull, FIFOSIZE);
  OS_InitSemaphore(&TxFIFO_semaFull, FIFOSIZE);
  OS_InitSemaphore(&RxFIFO_semaEmpty, 0);
  OS_InitSemaphore(&RxFIFO_semaEmpty, 0);
...
//  OS_Wait(&RxFIFO_semaFull);
//  OS_bWait(&RxFIFO_semaMutex);
  char letter;
  while(((UART0_FR_R&UART_FR_RXFE) == 0) && (RxFifo_Size() < (FIFOSIZE - 1))){
    letter = UART0_DR_R;  
    RxFifo_Put(letter);
  }
//  OS_bSignal(&RxFIFO_semaMutex);
//  OS_Signal(&RxFIFO_semaEmpty);
}
// copy from software TX FIFO to hardware TX FIFO
// stop when software TX FIFO is empty or hardware TX FIFO is full
void static copySoftwareToHardware(void){
//  OS_Wait(&TxFIFO_semaEmpty);
//  OS_bWait(&TxFIFO_semaMutex);
  char letter;
  while(((UART0_FR_R&UART_FR_TXFF) == 0) && (TxFifo_Size() > 0)){
    TxFifo_Get(&letter);
    UART0_DR_R = letter;
  }
//  OS_bSignal(&TxFIFO_semaMutex);
//  OS_Signal(&TxFIFO_semaFull);
}
