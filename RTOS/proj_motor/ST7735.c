...
void ST7735_strFmt(char* str, int32_t value, char* strBuf) {
	sprintf(strBuf, "%s %d", str, value);
}

// *************** ST7735_Message_Line ********************
// Put string and value to the LCD
// Device specified by 0 as top screen and 1 as bottom screen
// All other device inputs are not accepted
// Inputs: device, line, string
// Outputs: none
void ST7735_Message_Line(int device, int line, char *string) {
	if(device != 0 && device != 1) return;
	if(line < 0 && line > 7) return;
	OS_Wait(&lcdLock);
	ST7735_SetCursor(0, line + (8 * device));
	ST7735_OutString("                    ");
	Output_Color((device) ? ST7735_GREEN : ST7735_RED);
	ST7735_SetCursor(0, line + (8 * device));
...
// Device specified by 0 as top screen and 1 as bottom screen
// All other device inputs are not accepted
// Inputs: device, line, string, value
// Outputs: none
void ST7735_Message(int device, int line, char *string, int value) {
	char strBuf[21];
	ST7735_strFmt(string, value, strBuf);
	ST7735_Message_Line(device, line, strBuf);
}
