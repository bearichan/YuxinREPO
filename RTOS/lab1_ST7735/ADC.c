...
AddIndexFifo(ADC,FIFO_MAX_SIZE,uint32_t,FIFOSUCCESS,FIFOFAIL);

uint32_t g_numberOfSamples;
//code added by Xiangxing Yang
int ADC_Start_Collect(uint32_t channelNum, uint32_t fs,uint32_t numberOfSamples){

  volatile uint32_t delay;
  uint32_t period;
  period = 80000000 / fs;
  g_numberOfSamples = numberOfSamples;
  // **** GPIO pin initialization ****
  switch(channelNum){             // 1) activate clock
    case 0:
    case 1:
  ...
  DisableInterrupts();
  SYSCTL_RCGCADC_R |= 0x01;     // activate ADC0 
  //SYSCTL_RCGC0_R |= 0x00010000; // activate ADC0 in legacy mode 
  ...
  memset(ADCFifo,0,sizeof(ADCFifo));
  return 0;
}

//uint16_t ADC_In(void){
//  uint16_t data;
//  ADC0_PSSI_R = 0x0008;            // 1) initiate SS
...
int ADC_Status(void){
  ...
  if(ADCFifo_Size() < g_numberOfSamples){
    return 1;
  }
  else{
    return 0;
...
void ADC0Seq3_Handler(void){
  ...
  PF2 ^= 0x04;
	ST7735_Message(0,2, "ADC value: ", ADC0_SSFIFO3_R);
  if(ADC_Status()){
