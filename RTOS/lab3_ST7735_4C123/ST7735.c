...
//coded by Yuxin Wang
Sema4Type ST7735semaPt;
void ST7735_Initsema(void)
{
	OS_InitSemaphore(&ST7735semaPt, 1); 
}

void ST7735_Message (int device, int line, char *string, int32_t value){
  OS_Wait(&ST7735semaPt);
  if(line >= 8) ST7735_OutString("exceed line number");
  int y_start = (device==0) ? 0:8;   //if device == 0. use top half, otherwise use bottom half
  StY = y_start + line;
  StX = 0;
  ST7735_DrawString(StX, StY, string, StTextColor);
  StX += strlen(string);
  Messageindex = 0;
  fillmessage(value);
  Message[Messageindex] = 0; // terminate
  ...
  if(StX>20){
    ...
    ST7735_DrawCharS(StX*6,StY*10,'*',ST7735_RED,ST7735_BLACK, 1);
	  ...
	  for(int i=StX; i<=20; i++)
	  {
      ST7735_DrawString(i,StY," ",StTextColor);
	  }
  }
  OS_Signal(&ST7735semaPt);
}
