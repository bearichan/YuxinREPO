...
void Interpreter(void){
  ...
  while(1)
  {
    ...
    else if(strncmp(bufPt+1, "format", 6) == 0){
      eFile_Format();
      OutCRLF();
    }
    else if(strncmp(bufPt+1, "pdir", 4) == 0){
      eFile_Directory(UART_OutChar);
      OutCRLF();
    }
    else if(strncmp(bufPt+1, "pfile", 5) == 0){
//      UART_InToken(string, 8);
      eFile_Init();
      sprintf(string,bufPt+7);
      OutCRLF();
      eFile_Print(bufPt+7, UART_OutChar);
//      char data;
...
//      }
    ...
    else if(strncmp(bufPt+1, "dfile", 5) == 0){
//      UART_InToken(string, 8);
      eFile_Delete(bufPt+7);
      OutCRLF();
    }
    else{
...
void TestDisk(void){  DSTATUS result;  unsigned short block;  int i; unsigned long n;
  ...
  ST7735_Message(0, 1, "eDisk test    ", 1);
...
//    PD2 = 0x00;
  ...
  ST7735_Message(0, 1, "eDisk successful ", 1);
...
//  if(eFile_Create("file1"))     diskError("eFile_Create",0);
  ...
  printf("2.show directory contents\n\r");
  eFile_Directory(&UART_OutChar);
  //3.print file data
  printf("3.print file data\n\r");
  char data;
...
//  if(eFile_Create("file1"))     diskError("eFile_Create",0);
  ...
  PD2 = 0x04;     // PF2 high for one block read
  for(i=0;i<1000;i++){
    if(eFile_Write('0'+i%9))   diskError("eFile_Write",i);
    if(i%52==51){
      if(eFile_Write('\n'))     diskError("eFile_Write",i);  
      if(eFile_Write('\r'))     diskError("eFile_Write",i);
    }
  ...
  PD2 = 0x00;     
  if(eFile_WClose())            diskError("eFile_WClose",0);
  //6.create a file with same name
  printf("6.create a file with same name\n\r");
  if(eFile_Create("file1"))     diskError("eFile_Create",0);
  //7.closed unopened file
  printf("7.closed unopened file\n\r");
  if(eFile_WClose())            diskError("eFile_WClose",0);
  //8.append
  printf("8.append\n\r");
  if(eFile_WOpen("file1"))      diskError("eFile_WOpen",0);
...
//  if(eFile_Create("file1"))     diskError("eFile_Create",0);
  ...
  PD2 = 0x04;     // PF2 high for one block read
  for(i=0;i<4000;i++){
    if(eFile_Write('h'+i%6))   diskError("eFile_Write",i);
    if(i%52==51){
      if(eFile_Write('\n'))     diskError("eFile_Write",i);  
      if(eFile_Write('\r'))     diskError("eFile_Write",i);
    }
...
//  if(eFile_Format())
//  {
//    ST7735_Message(1, 1, "Format fail", 0);
//  }
//  else
//  {
//    ST7735_Message(1, 1, "Format success", 0);
//  }
  NumCreated += OS_AddThread(&Checkout_TestFile,128,1);  
